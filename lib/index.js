 ...
 *
 *  Node 内置 + lib/core.js（纯函数，可测试）+ @deepseek-ai/dsh-tools defineTool
 *  （P1-05：三工具改走受校验注册路径，运行时入参按 JSON Schema 拦截越界/错类型）——JS 直出模式。
...
import {
  ...
  isReadTool, extractText, sha1, targetOf, duplicateOf,
...
} from './core.js' // 不带查询串：ESM 按 URL 缓存，?v= 会造成双模块实例
import { defineTool } from '@deepseek-ai/dsh-tools' // P1-05：受校验工具注册路径（入参 JSON Schema 拦截）
...
function registerToolSafe(ctx, tool) {
  try {
    // P1-05：tool 已是 defineTool 产物（入参在 execute 前按 JSON Schema 受校验，
    // 越界/错类型参数抛 ToolArgsError 并被 dispatch 转为 isError 结果）。
    // defineTool 的隐式参数根为开放对象（不含 additionalProperties），此处补回
    // SPEC §8 严格边界（additionalProperties:false），与旧 toJsonSchema 语义一致。
    if (tool.parameters && typeof tool.parameters === 'object') tool.parameters.additionalProperties = false
